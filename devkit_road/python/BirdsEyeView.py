...
class BirdsEyeView(object):
    ...
    def transformImage2BEV(self, inImage, out_dtype = 'f4'):
        ...
        assert self.im_u_float.size != 0
        assert self.im_v_float.size != 0
        assert self.bev_x_ind.size != 0
        assert self.bev_z_ind.size != 0
        
        
        if len(inImage.shape) > 2:
            self.bevParams.bev_size=(int(self.bevParams.bev_size[0]),int(self.bevParams.bev_size[1]))
